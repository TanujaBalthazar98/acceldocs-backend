...
def test_public_page_rewrites_google_doc_and_legacy_links(client, db, monkeypatch):
    ...
    html = resp.text
    canonical = f'/docs/link-org/p/{target.id}/{target.slug}'
...
def test_public_page_rewrites_cross_visibility_links_to_correct_docs_root(client, db, monkeypatch):
    ...
    assert resp.status_code == 200
    html = resp.text
    assert f"/internal-docs/cross-vis-org/p/{internal_target.id}/{internal_target.slug}" in html
    assert f"/external-docs/cross-vis-org/p/{external_target.id}/{external_target.slug}" in html
...
def test_internal_link_target_shows_access_gate_instead_of_raw_error(client, db, monkeypatch):
    ...
    expected_internal_link = f"/internal-docs/gate-org/p/{internal_target.id}/{internal_target.slug}"
...
def test_legacy_slug_route_redirects_via_page_redirect(client, db, monkeypatch):
    ...
    assert resp.headers["location"] == f"/docs/redirect-org/p/{page.id}/{page.slug}"
